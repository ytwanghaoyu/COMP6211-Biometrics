...
try:
    ...
    testImagePath=os.listdir('G:/openpose-master/openpose-master/build_CPU/examples/tutorial_api_python/testimage')
    trainImagePath.sort(key= lambda x:int(x[:-4]))
    testImagePath.sort(key= lambda x:int(x[:-4]))
    ...
    for item in testImagePath:
        print(item)
        args[0].image_path = '../testimage/' + item
        ...
        #cv2.imshow('OutputImageTest/imageWithKeyPoint'+str(args[0].image_path), datum.cvOutputData)
        cv2.waitKey(0)
        cv2.imwrite('../OutputImageTest/Test'+str(index)+'.jpg', datum.cvOutputData)
        index+=1
    ...
    dfbodykeypoints.to_csv('test_bodykeypoints.csv', index=False)
    dffacekeypoints.to_csv('test_facekeypoints.csv', index=False)
